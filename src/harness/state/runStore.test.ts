...
test("run store serializes concurrent read-modify-write mutations", async () => { root = await mkdtemp(join(tmpdir(), "harny-run-")); const store = new RunStore(root, "task"); const value = snapshot(root); value.inputs.count = 0; await store.create(value); await Promise.all(Array.from({ length: 10 }, () => store.mutate((run) => { run.inputs.count = Number(run.inputs.count) + 1; }))); expect((await store.load())?.inputs.count).toBe(10); });
