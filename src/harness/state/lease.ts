...
export async function acquireRunLease(root: string, key: string, ownerRunId: string, pid = process.pid): Promise<RunLease> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try { handle = await open(path, "wx"); await handle.writeFile(JSON.stringify({ key, ownerRunId, pid, acquiredAt })); await handle.close(); handle = undefined; return { key, ownerRunId, acquiredAt, release: async () => { const current = JSON.parse(await readFile(path, "utf8")); if (current.ownerRunId === ownerRunId) await unlink(path); } }; }
    catch (error) {
