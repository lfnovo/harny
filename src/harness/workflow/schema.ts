...
const PredicateSchema: z.ZodType<WorkflowPredicate> = z.lazy(() =>
  z.union([
    z.object({ equals: z.tuple([z.unknown(), z.unknown()]) }),
...
const CommandNodeSchema = z.object({ ...CommonNode, type: z.literal("command"), command: z.array(z.string()).min(1) });
const HumanNodeSchema = z.object({ ...CommonNode, type: z.literal("human"), question: z.string().min(1), fallback: z.string().optional() });
const CommitNodeSchema = z.object({ ...CommonNode, type: z.literal("commit"), message: z.string().min(1), changeset: z.string().min(1) });
const PullRequestNodeSchema = z.object({ ...CommonNode, type: z.literal("pull_request"), title: z.string().min(1), body: z.string().default(""), base: z.string().default("main"), head: z.string().min(1), draft: z.boolean().default(true), existing: z.enum(["allow", "require", "forbid"]).default("allow") });
const CancelNodeSchema = z.object({ ...CommonNode, type: z.literal("cancel"), reason: z.string().min(1) });
