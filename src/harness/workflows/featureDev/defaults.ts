...
4. DO NOT edit the plan file (.harny/<task>/plan.json). The harness owns it.
...
- **Independence requirement:** your exercise must be YOUR OWN invocation. If the developer phase produced smoke-test artifacts (e.g. a /tmp/harny-e2e-* directory from a prior run), those are ONE input to your evidence, never a substitute for your own run. For end-to-end ACs, YOU run the command yourself. For concurrent-run ACs, YOU start the concurrent processes yourself. For "the harness runs in mode X" ACs, YOU invoke the harness in mode X yourself. Independent execution catches bugs that only surface under your specific environment or timing and is the only way to protect against the developer's own blind spots.
- **Be EFFICIENT about empirical exercise — recursion is expensive.** When the system you are validating IS the harness itself, every nested \`bun bin/harny.ts\` invocation is a full planner+developer+validator cycle that takes 5-15 minutes. Multiplying these blows up wall clock geometrically. Rules of efficiency:
  ...
  - **Plan your nested invocations BEFORE running them.** Write down (in your own reasoning) which UNIQUE nested run you intend, and which ACs it will cover. Avoid one-AC-per-run drift. If you find yourself reaching for a 2nd nested run, stop and ask: "could I cover this AC by inspecting the FIRST run's artifacts (commits, files, plan.json, state.json:history)?" — usually yes.
  - **Use unique task slugs across nested runs in the same validation.** \`harny/<slug>\` branch + \`.harny/<slug>/\` state dir collide if you reuse the slug. Suffix with the AC label or a counter (\`smoke-ac1\`, \`smoke-ac2\`) when truly running multiple, OR run \`bun bin/harny.ts clean <slug>\` between invocations.
