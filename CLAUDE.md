...
- `src/harness/engine/runtime/runEngineWorkflow.ts` — `runEngineWorkflow(workflow, { cwd, taskSlug, runId, log? })`: creates an XState actor from `workflow.machine`, subscribes to snapshots, resolves when final state reached, returns `{ status: 'done'|'failed', finalContext, error? }`. 60s hard deadline.
