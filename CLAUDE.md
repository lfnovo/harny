...
- `src/runner.ts` — CLI entry. Flags: `--assistant <name>` (opt-in; defaults to `process.cwd()`), `--task <slug>`, `--workflow <name>` (default `feature-dev`), `--isolation <worktree|inline>`, `--mode <interactive|silent|async>`, `-v/--verbose`, `--quiet`. Subcommands (recognized when first positional matches): `clean <slug>`, `ls [--status X] [--cwd X] [--workflow X]`, `show <runId> [--tail] [--since=<duration>]` (run_id prefix ≥8 chars accepted; `--tail` streams active phase activity, `--since=Ns|Nm|Nh` backfills recent events), `answer <runId>` (recognized but stubbed — see "Run modes"), `ui [--port=N] [--no-open]` (boots local viewer + opens browser).
- `src/harness/orchestrator.ts` — generic interpreter. Resolves workflow from registry, sets up git/branch/worktree per workflow flags, then calls `runEngineWorkflow` (XState createActor). All workflows are engine workflows; the legacy `Workflow.run(ctx)` path was removed 2026-04-23. Idempotent rerun guard: refuses to clobber an existing `state.json` (done/failed = exit gracefully, running = error with stale-pid hint, waiting_human = error suggesting `harny clean <slug>`).
...
- `src/viewer/` — read-only HTTP wrapper + vanilla JS SPA for browsing runs. Boot via `harny ui`. `server.ts` exposes `/api/{health,config,assistants,runs,runs/:cwdHash/:slug,runs/:cwdHash/:slug/git-log,runs/:cwdHash/:slug/sibling-branches}` and serves `index.html`. Phoenix project name → GraphQL ID lookup runs server-side (Phoenix has no CORS) and is cached 30s.
...
- `src/harness/askUser.ts` — `resolveAnswer` + `runAskUserQuestionTTY` (used by `sessionRecorder` in interactive mode) and the `AskUserQuestion*` types reused by `harny show`. No error classes here anymore: the legacy `SilentModeError` / `PausedForUserInputError` died with the legacy orchestrator path (2026-04-23).
- `src/harness/git.ts` — preconditions, branch creation, commit helpers, worktree primitives.
- `src/harness/guardHooks.ts` — `PhaseGuards` (`{readOnly?, noPlanWrites?, noGitHistory?}`) + `buildGuardHooks`. Escape hatch for paths outside phase cwd (e.g., `/tmp/harny-e2e-*`) is honored. **Active in engine workflows as of 2026-04-23**: `DEFAULT_DEVELOPER` sets `{noPlanWrites, noGitHistory}`, `DEFAULT_VALIDATOR` sets `{readOnly}`. `readOnly` covers `Write|Edit|MultiEdit|NotebookEdit` only — Bash mutations (`echo >`, `sed -i`, `rm`) are NOT blocked (documented gap; validator needs Bash to run tests). Unit probe: `scripts/probes/hooks/01-guardhooks.ts`.
- `src/harness/clean.ts` — `clean <slug>` subcommand: idempotent worktree + branch + state dir cleanup.
- `src/harness/workflows/` — workflow catalog. Registry in `index.ts` exports `getWorkflow(id): WorkflowDefinition<any>`. Since the legacy path removal (2026-04-23) all registered workflows are engine workflows, so the registry has one shape and the old `isEngineWorkflow` type guard is gone. New workflows live in `src/harness/engine/workflows/` and are imported into this registry. `composeCommit.ts` is the shared commit-message composer (do not move or delete).
...
- `src/harness/workflows/composeCommit.ts` — `composeCommitMessage({ devMessage, taskId, role, evidence })`: strips pre-existing `task=<id>` trailers from devMessage before appending exactly one — prevents the duplicate-trailer bug.
...
- **silent**: `AskUserQuestion` is stripped from `allowedTools` before `query()`; any stray call is denied defensively.
- **async**: `AskUserQuestion` calls park as `state.pending_question` and the run exits `waiting_human`. **Resume is not implemented for engine workflows** (the only kind today) — `harny answer <runId>` is recognized but returns an error pointing at `harny clean <slug>`. Parked runs are discard-only for now; implementing resume in the engine is a deliberate open item, not a forgotten feature.
...
- State inspection: `harny ls [--status waiting_human]`, `harny show <runId>` (prefix ≥8 chars), `harny clean <slug>`. `harny answer <runId>` exists but is a stub (engine resume NYI).
