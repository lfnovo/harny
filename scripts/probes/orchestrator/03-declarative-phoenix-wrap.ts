/**
 * Probe: declarative-path-phoenix-wrap — static observability wiring check.
 *
 * Verifies that the declarative runtime remains inside the Phoenix run span.
 *
 * RUN
 *   bun scripts/probes/orchestrator/03-declarative-phoenix-wrap.ts
 */
...
async function runProbes(): Promise<void> {
  // Scenario: declarative-path-phoenix-wrap
  {
    const name = 'declarative-path-phoenix-wrap';
    try {
      await Promise.race([
        (async () => {
          ...
          if (!window.includes('runNextFeatureDev') || !window.includes('runDeclarativeWorkflow')) {
            throw new Error(
              'declarative runners not found within 2000 chars after setupPhoenix({ in orchestrator.ts',
            );
          }
        })(),
