...
try {
  await Promise.race([
    (async () => {
      ...
      // feature-dev appends 4 PhaseEntry (planner, developer, validator, committing);
      // only the 3 SDK-backed phases emit phase_end history events. See add56e6.
      ...
      if (capturedPhases.length !== 4) {
        throw new Error(`expected phases.length === 4, got ${capturedPhases.length}`);
      }

      const phaseEndEvents = capturedHistory.filter((e: any) => e.event === 'phase_end');
      if (phaseEndEvents.length !== 3) {
        throw new Error(
          `expected === 3 phase_end history events, got ${phaseEndEvents.length}`,
        );
      }
